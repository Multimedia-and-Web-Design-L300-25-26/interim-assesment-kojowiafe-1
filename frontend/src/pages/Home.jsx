...
function Home() {
  const { coins, gainers, newListings } = useLivePrices() || { coins: [], gainers: [], newListings: [] };
  const [activeTab, setActiveTab] = useState('tradable');
  
  let listToUse = coins;
  if (activeTab === 'gainers') listToUse = gainers;
  if (activeTab === 'new') listToUse = newListings;

  const topCryptos = listToUse.slice(0, 4);
